...
export default function StoryCard({ story, layout = 'grid' }) {
  ...
  if (layout === 'row') {
    return (
      <Link
        to={href}
        className="flex items-start gap-5 border-b border-gray-100 py-5 hover:bg-gray-50"
        data-slug={story.slug}
      >
        <StoryThumb story={story} size="md" />
        <div className="min-w-0 flex-1">
          <h3 className="text-xl font-bold text-jw-ink">{story.title}</h3>
          ...
          <p className="mt-1 text-gray-600">{story.blurb}</p>
  ...
  return (
    <Link to={href} className="group block" data-slug={story.slug}>
      ...
      <h3 className="mt-3 text-xl font-bold text-jw-ink group-hover:text-jw-blue">
        {story.title}
      </h3>
      <p className="mt-1 text-gray-600">{story.blurb}</p>
      <div className="mt-1 text-sm text-gray-400">{story.duration}</div>
    </Link>
  );
}
